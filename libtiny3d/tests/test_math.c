...
int main() {
    ...
    print_mat4("Scale Matrix", &scale_matrix);
    ...
    // Applied as: Vert_world = Translation * Rotation * Scale * Vert_model
    ...
    // 4. Create a projection matrix
    // mat4_t projection_matrix = mat4_frustum_asymmetric(-1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 100.0f);
    ...
    // Full transformation: Vert_clip = Projection * Model * Vert_model
    ...
    for (int i = 0; i < 8; ++i) {
        // We need to apply MVP and then perspective divide to get Normalized Device Coordinates (NDC)
        // mat4_transform_point handles the perspective divide internally.
        transformed_vertices[i] = mat4_transform_point(&mvp_matrix, &cube_vertices[i]);
        char v_name[10];
        sprintf(v_name, "V%d_clip", i);
        print_vec3(v_name, &transformed_vertices[i]);
    }

    // Note: The output of mat4_transform_point is already in NDC-like coordinates (x,y,z after divide).
    // To get screen coordinates, you'd typically map x,y from [-1,1] to [0, screen_width/height].
    // e.g., screen_x = (v.x + 1.0) * 0.5 * screen_width;
    // e.g., screen_y = (1.0 - v.y) * 0.5 * screen_height; (1.0 - v.y because y often points down in screen space)

    printf("\n--- Quaternion Tests ---\n");
    vec3_t axis = vec3_create_cartesian(0,1,0); // Y-axis
    vec3_normalize(&axis);
    quat_t q_rot = quat_from_axis_angle(axis, M_PI/2.0f); // 90 deg rotation around Y
    ...
    quat_t q_ident = {0,0,0,1}; // Identity quaternion
    quat_t q_slerp_res = quat_slerp(q_ident, q_rot, 0.5f); // Halfway rotation
